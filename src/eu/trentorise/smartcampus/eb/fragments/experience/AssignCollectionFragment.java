...
public class AssignCollectionFragment extends SherlockDialogFragment {
	...
	public void onSaveInstanceState(Bundle out) {
		...
		out.putBooleanArray("selected", selected == null ? new boolean[0]
				: selected);
		out.putStringArrayList("selectedIds",
				selectedIds == null ? new ArrayList<String>()
						: new ArrayList<String>(selectedIds));
		out.putStringArray("items", items);
	}

	@Override
	public Dialog onCreateDialog(Bundle savedInstanceState) {
		final List<ExpCollection> collections = EBHelper.getUserPreference()
				.getCollections();
		...
		if (savedInstanceState != null) {
			...
			selectedIds = new HashSet<String>(
					savedInstanceState.getStringArrayList("selectedIds"));
			...
			if (getArguments().getStringArrayList(ARG_COLLS) != null)
				selectedIds
						.addAll(getArguments().getStringArrayList(ARG_COLLS));
		}
		...
		builder.setNeutralButton(android.R.string.ok,
				new DialogInterface.OnClickListener() {
					@Override
					public void onClick(DialogInterface dialog, int which) {
						// exp.setCollectionIds(new
						// ArrayList<String>(selectedIds));
						// Fragment f =
						// getFragmentManager().findFragmentById(containerId);
						AssignCollectionsCallback c = ((DialogCallbackContainer) getActivity())
								.getAssignCollectionsCallback();
						c.onCollectionsAssigned(getArguments()
								.getString(ARG_ID), new ArrayList<String>(
								selectedIds));
						dialog.dismiss();
					}
				});
		builder.setPositiveButton(R.string.dialog_collection_add,
				new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						...
						newCollFragment
								.setArguments(NewCollectionDialogFragment
										.prepare(null));
						newCollFragment.show(getActivity()
								.getSupportFragmentManager(), "exp_coll");
						//
						// NewCollectionDialogFragment.saveCollectionDialog(getActivity(),
						// null, new CollectionSavedHandler() {
						// @Override
						// public void onCollectionSaved(ExpCollection coll) {
						// //
						// exp.setCollectionIds(Collections.singletonList(coll.getId()));
						// ((AssignCollectionsCallback)getFragmentManager().findFragmentById(android.R.id.content)).onCollectionsAssigned(getArguments().getString(ARG_ID),
						// Collections.singletonList(coll.getId()));
						// }
						// }).show();
						dialog.dismiss();
					}
